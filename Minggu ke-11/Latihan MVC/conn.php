...
class Koneksi
{
    ...
    public function search()
    {
        ...
        $query = $this->db->prepare("SELECT * FROM sekolah".$cari."ORDER BY id DESC");
        $query->execute();
        $data = $query->fetchAll();
        return $data;
    }
    ...
    public function update($id,$nama,$alamat,$update_logo){
        $query = $this->db->prepare("UPDATE sekolah SET `nama`='" . $nama . "', `alamat`='" . $alamat . "' " . $update_logo . " WHERE `id` ='" . $id . "'");
    ...
    public function pdf()
    {
        $query = $this->db->prepare("SELECT * FROM sekolah WHERE id = '" . get('id') . "'");
        $query->execute();
